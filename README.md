...
- **可测试 + 可评测**：155 条单测全程不调用真实 LLM（mock / 脚本化假模型），无需 API Key；
...
共 **155 passed**。测试全程不调用真实 LLM（LLM 全部 mock 或用脚本化假模型），无需 API Key。
...
- [x] pytest 155 passed（mock LLM）+ Docker
