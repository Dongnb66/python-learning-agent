...
- **可测试 + 可评测**：156 条单测全程不调用真实 LLM（mock / 脚本化假模型），无需 API Key；
...
共 **156 passed**。测试全程不调用真实 LLM（LLM 全部 mock 或用脚本化假模型），无需 API Key。
...
- [x] pytest 156 passed（mock LLM）+ Docker
