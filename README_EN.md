...
- **Testable + evaluable**: 153 tests run with no real LLM calls and no API key; plus an **anti-hallucination evaluation suite** (`eval/bad_cases.json` + `scripts/run_eval.py`) — 6 normal + 3 adversarial cases / 4 assertion types quantifying "0 fabricated links, 100% verifiable sources", ready for CI
...
**153 passed.** No real LLM is called — no API key needed.
...
- [x] pytest 153 passed (mocked LLM, no API key needed) + Docker
